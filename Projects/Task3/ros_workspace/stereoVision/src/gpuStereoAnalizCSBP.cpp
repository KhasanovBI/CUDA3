...
static int StereoNDisp = 128;
static int StereoIters = 8;
static int StereoLevels = 4;
static int StereoNRPlane = 4;
...
void imageCallback(const sensor_msgs::ImageConstPtr& msg1, const sensor_msgs::ImageConstPtr& msg2)
{
	...
	csbp.ndisp = StereoNDisp;
	csbp.iters = StereoIters;
	csbp.levels = StereoLevels;
	csbp.nr_plane = StereoNRPlane;
...
int main(int argc, char **argv)
{
	...
	cv::createTrackbar("NDisp ", "Stereo Control", &StereoNDisp, 256);
	cv::createTrackbar("Iters", "Stereo Control", &StereoIters, 16);
	cv::createTrackbar("Levels", "Stereo Control", &StereoLevels, 8);
	cv::createTrackbar("NRPlane", "Stereo Control", &StereoNRPlane, 8);
