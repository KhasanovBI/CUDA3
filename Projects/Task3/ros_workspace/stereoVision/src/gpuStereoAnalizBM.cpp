...
static int StereoNDisp = 35;
static int StereoWinSize = 10;
static int StereoPresent = 2;
static int StereoTexTh = 5;
...
void imageCallback(const sensor_msgs::ImageConstPtr& msg1, const sensor_msgs::ImageConstPtr& msg2)
{
	...
	bm.ndisp = StereoNDisp;
	bm.winSize = StereoWinSize;
	bm.preset = StereoPresent;
	bm.avergeTexThreshold = StereoTexTh;
...
int main(int argc, char **argv)
{
	...
	cv::createTrackbar("NDisp ", "Stereo Control", &StereoNDisp, 2048);
	cv::createTrackbar("WinSize", "Stereo Control", &StereoWinSize, 50);
	cv::createTrackbar("Present", "Stereo Control", &StereoPresent, 1000);
	cv::createTrackbar("TexCH ", "Stereo Control", &StereoTexTh, 1000);
